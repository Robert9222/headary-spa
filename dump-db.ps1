# dump-db.ps1 - eksport bazy PostgreSQL do exports/<db>-<stamp>.sql
# Czyta DB_* z backend/.env. Wymaga pg_dump w PATH lub w 'C:\Program Files\PostgreSQL\<v>\bin'.
...
if ($envVars.DB_CONNECTION -ne 'pgsql') { throw "Ten skrypt wspiera tylko pgsql (jest: $($envVars.DB_CONNECTION))" }
...
try {
    & $pgDump -h $envVars.DB_HOST -p $envVars.DB_PORT -U $envVars.DB_USERNAME -d $envVars.DB_DATABASE --clean --if-exists --no-owner --no-privileges -E UTF8 -f $out
    if ($LASTEXITCODE -ne 0) { throw "pg_dump zakonczyl sie kodem $LASTEXITCODE" }
} finally {
    Remove-Item Env:PGPASSWORD -ErrorAction SilentlyContinue
}
$sizeKB = [math]::Round((Get-Item $out).Length / 1KB, 1)
Write-Host "[OK] Dump utworzony: $out ($sizeKB KB)" -ForegroundColor Green
